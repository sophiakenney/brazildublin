...
filt <- read.delim("readqc/k2filtstats.txt", sep = "\t")
...
# top genera for even the lower ppn ones is Salmonella and they are barely under 90p
...
raw <- raw %>%
  ...
  mutate(read = case_when(
    str_detect(file, "_1.fastq.gz") ~ "forward",
    str_detect(file, "_2.fastq.gz") ~ "reverse"
  )) %>%
...
trim <- trim %>%
  mutate(ID = str_split_fixed(file, "\\_", 2)[,1]) %>%
  filter(str_detect(file, "P.fq.gz")) %>%
  mutate(read = case_when(
